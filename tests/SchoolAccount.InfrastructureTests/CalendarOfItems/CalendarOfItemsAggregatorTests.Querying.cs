...
public partial class CalendarOfItemsAggregatorTests
{
    ...
    public async Task Running_a_standard_query_it_will_always_succeed(int poolSize, int pageSize, int pageNumber, CalendarOfItemsViewModes mode)
    {
        // Arrange
        ...
        result.Value.Payload.Count.Should().Be(Math.Min(poolSize, pageSize), because: "Should always match the query limit");
    ...
    public async Task When_filtering_a_query_with_assignable_properties_the_query_factory_is_correctly_called()
    {
        ...
        var inboundFilter = CalendarOfItemsFilterExtensions.Create(
            FilterRequestExtensions.Create("Name")
                .WithValue("Test")
        );
        var queryFactory = Make.Factory.Query(
            CalendarOfItemsQueryTypes.SubTask, 
        ...
        // Assert
        queryFactory.Received(1).Query(
            Arg.Is<CalendarOfItemsFilter>(f => f.All(x => x.Field == "Name")),
            Arg.Any<FieldSelectorMapping>());
    }
}