...
his influential 2012 article [The Clean Architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html). He also published the book [Clean Architecture: A 
Craftsman's Guide to Software Structure and Design](https://www.oreilly.com/library/view/clean-architecture-a/9780134494166/), which expands on these principles in depth.
...
This application is based primarily on the practical .NET application of Clean Architecture by **[Milan Jovanović](https://www.milanjovanovic.tech/)** -
Software Architect and Microsoft MVP.
---

## Uncle Bob's Original Vision

![Uncle Bob's Clean Architecture concentric rings diagram](images/uncle_bob_clean_architecture.jpg)
...
![Clean Architecture layers diagram](images/clean_architecture_layers.png)
...
- **Validation** - input validation via FluentValidation pipeline behaviours
- **Pipeline behaviours** - cross-cutting concerns like logging and transaction handling
...
- **Background jobs**
- **Authentication integrations** (e.g. DfE Sign-In)

Infrastructure depends on both Domain and Application but is never referenced by them.

### 4. Presentation

The entry point to the system - the Web project. In this solution that is `SchoolAccount.Web.Connect`.

Contains:
- **Controllers** - thin wrappers that dispatch Commands and Queries
...
| Infrastructure | `SchoolAccount.Infrastructure` + `SchoolAccount.Integration.DfESignIn` |
| Presentation | `SchoolAccount.Web.Connect` |
| Shared kernel | `SchoolAccount.Kernel` |

`SchoolAccount.Kernel` contains primitives used across all layers (`Result<T>`, `Error`, `Entity`, `IUserContext`) and sits outside the ring model - it has no dependencies itself and can be referenced by any layer.

The Application layer organises features using **package by feature** under `src/SchoolAccount.Application/Features/`.
