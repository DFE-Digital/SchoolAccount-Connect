...
{
    public static Func<TRow, bool> IsWithinDateRange<TRow>(DateOnly rangeStart, DateOnly rangeEnd)
        where TRow : IQueryRow
    {
        return x => x.SortDate.HasValue && x.SortDate.Value >= rangeStart && x.SortDate.Value <= rangeEnd;
    }

    public static Func<TRow, bool> IsWithinDateRange<TRow>(DateOnlyRange range)
        where TRow : IQueryRow
    {
