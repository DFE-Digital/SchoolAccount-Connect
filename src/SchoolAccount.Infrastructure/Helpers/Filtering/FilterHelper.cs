...
public static class FilterHelper
{
    internal static Expression Condition(Expression property, FilterRequest node)
    {
        return node.Operator switch
        {
            ...
            ComparisonType.LessThan => ExpressionBuilders.Compare(ExpressionType.LessThan, property,
                node.Value),
            ComparisonType.LessThanOrEquals => ExpressionBuilders.Compare(ExpressionType.LessThanOrEqual,
                property, node.Value),
            ComparisonType.GreaterThan => ExpressionBuilders.Compare(ExpressionType.GreaterThan, property,
                node.Value),
            ComparisonType.GreaterThanOrEquals => ExpressionBuilders.Compare(ExpressionType.GreaterThanOrEqual,
                property, node.Value),
            ...
            ComparisonType.NotContains => Expression.Not(ExpressionBuilders.Contains(property, property.Type,
                node.Value)),
    ...
    internal static Expression Build<T>(ParameterExpression param, FilterRequest node, FieldSelectorMapping map)
    {
        ...
        var property = new ParameterReplaceVisitor(selector.Parameters[0], param)
            .Visit(selector.Body)!;

        return Condition(property, node);
    }

    public static IQueryable<T> Apply<T>(this IQueryable<T> query, Collection<FilterRequest> filters, FieldSelectorMapping map)
    {
